...
import { ChevronLeft, ChevronRight, Instagram, MessageCircle, Moon, Sun, Languages, Calendar, MapPin, Mail } from "lucide-react";
...
function StreamingCarousel({ posts, a11yTexts }: { posts: string[], a11yTexts: any }) {
  ...
  return (
    <div className="relative w-screen left-1/2 right-1/2 -mx-[50vw] mb-8">
      ...
      >
        {/* Correção do erro TS(2322): Usando tag style padrão com dangerouslySetInnerHTML */}
        <style dangerouslySetInnerHTML={{ __reactStyles: `
...
const translations = {
  pt: {
    ...
    hero: { 
      ...
      btn1: "Solicitar Orçamento via WhatsApp",
    ...
    services: {
      ...
      singlePrice: "A partir de R$ 150,00",
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 relative font-sans">
      ...
      <main className="relative z-10">
        {/* ... Restante do conteúdo do seu componente ... */}
        {/* Adicionei aqui a seção de localização e cursos conforme o arquivo original */}
        
        <section id="location" className="py-20 container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div className="space-y-4">
              <h3 className="text-2xl font-serif font-bold" style={{ color: colors.textPrimary }}>{t.location.addressTitle}</h3>
              <p style={{ color: colors.textSecondary }}>Rua Dona Maria Resende, 171 - Vila Garcia, Patos de Minas - MG</p>
              <div className="flex gap-4">
                <a href="https://wa.me/553498109317" className="px-6 py-2 rounded-lg font-semibold" style={{ background: '#C9A961', color: 'white' }}>{t.location.btn1}</a>
                <a href="https://instagram.com/italofreitasmakeup" className="px-6 py-2 rounded-lg font-semibold border-2" style={{ borderColor: '#C9A961', color: '#C9A961' }}>{t.location.btn2}</a>
              </div>
            </div>
            <div className="h-[400px] rounded-lg overflow-hidden shadow-lg border" style={{ borderColor: colors.border }}>
              <iframe
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {t.courses.list.map((course ) => (
                <div key={course.title} className="p-6 rounded-lg shadow-lg transition-all hover:shadow-xl" style={{ background: colors.cardBg, border: `1px solid ${colors.border}` }}>
                  <h3 className="text-xl font-serif font-bold mb-2" style={{ color: colors.textPrimary }}>{course.title}</h3>
                  <p className="mb-4 text-sm" style={{ color: colors.textSecondary }}>{course.desc}</p>
